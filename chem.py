...
class Molecule:
    ...
    def __init__(self,data = None):
        ...
        if data == None:
            ...
            if data[-4:] == '.xyz':
                with open(data) as f:
                    molecule, info = process.read_molecule(f)
                try:
                    energy = float(info[0])
                    self.energy = energy
                except:
                    self.energy = None
                self.atom_list = molecule.atom_list
                # At least make adjacency
                self.adj_matrix = process.get_adj_matrix_from_distance(self,coeff)
            elif data[-4:] == '.com':
                with open(data) as f:
                    molecule, info = process.read_molecule(f,'com')
                self.atom_list = molecule.atom_list
                self.chg = molecule.chg
                ...
                try:
                    ...
                    #        add_hydrogen = False
                    #        break
                    ace_mol = process.get_ace_mol_from_rd_mol(rd_mol,add_hydrogen=add_hydrogen)
                except:
                    ace_mol = None
        ...
        elif type(data) is dict: # At least graph information is given ... If not, use xyz information
            atom_feature = {}
            # Check z
            if 'z' not in data: 
                atom_list = None
            ...
            # Check charge and bo_matrix ...
            if 'chg' in data and data['chg'] is not None:
                chg = data['chg']
            elif 'atom_chg' in data and data['atom_chg'] is not None:
                atom_feature['atom_chg'] = data['atom_chg']
                chg = int(np.sum(atom_feature['atom_chg']))
            if 'bo' in data and data['bo'] is not None:
                self.bo_matrix = data['bo']
            elif 'adj' in data and data['adj'] is not None:
            ...
            self.atom_list = atom_list
            self.multiplicity = data[3]

            if type(data[2]) is int: # (z_list, adj_matrix, chg, multiplicity)
                self.adj_matrix = data[1]
                self.chg = data[2]
            else: # (z_list, bo_matrix, chg_list, multiplicity)
                self.bo_matrix = data[1]
                if data[2] is not None:
                    self.atom_feature['chg'] = data[2]
                    self.chg = int(np.sum(data[2]))
    ...
    def set_atom_list(self,atom_list):
        #self.reset_molecule()
        #print ('Strongly suggest to generate a new molecule')
        #print ('Molecule is reset to the initialized atom list')
        self.atom_list = atom_list
    ...
    def set_smiles(self,smiles):
        #self.reset_molecule()
        #print ('Strongly suggest to generate a new molecule')
        #print ('Molecule is reset to None')
        self.smiles = smiles
    ...
    def set_center_of_mass(self,center_of_mass):
        ...
        #self.molecule_feature['com'] = center_of_mass

    def set_center(self,center):
        self.center = center
        #self.molecule_feature['center'] = center
    ...
    def initialize(self,method='scipy'):
    ...
    def get_smiles_with_rdkit(self,include_hydrogen = True,atom_mapping = False):
    ...
    def get_atom_id_list(self):
        ...
        distance_matrix = np.where(distance_matrix>5000,-1,distance_matrix)
        
        n = len(self.atom_list)
        z_list = self.get_z_list()
        z_matrix = np.repeat(z_list,n)
        z_matrix = z_matrix.reshape((n,n)) ** 2
        score_matrix = (z_matrix * z_matrix.T) * (distance_matrix + 1) ** 2
    ...
    def get_molecule_id(self):
        ...
        molecule_id = (chg + 13) * connectivity_id
        return molecule_id

    def get_id(self,method='ace'):
        if method == 'ace': 
            if self.molecule_id is not None:
                return self.molecule_id
            formula_id = self.get_formula_id()
            molecule_id = self.get_molecule_id()
            return f'{formula_id}_{molecule_id}'
        else:
            rd_mol = self.get_rd_mol()
            return Chem.inchi.MolToInchi(rd_mol)
    ...
    def get_matrix(self,type_of_matrix = 'bo'):
        ...
        if type_of_matrix == 'bo':
            ...
            adj_matrix = self.get_adj_matrix()
            n = len(self.atom_list)
            distance_matrix = 100000 * np.ones((n,n))
            np.fill_diagonal(distance_matrix,0)
            update_matrix = np.identity(n)
            for d in range(n):
                cur_d = d + 1
                update_matrix = np.matmul(update_matrix,adj_matrix)
                d_update_matrix = cur_d * np.where(update_matrix>0,1,100000)
    ...
    def compute_chg_and_bo(self,method='SumofFragments'):
        chg = self.get_chg()
        if chg is None:
            print ('Cannot make RDKit molecule because total charge is not specified !!!')
            return None, None
        chg_list, bo_matrix = process.get_chg_and_bo(self,chg,method=method)
    ...
    def is_same_molecule(self,molecule,option=False):
        ...
        #self.c_eig_list = c_eig_list1
        #molecule.c_eig_list = c_eig_list2
        delta_c_eig_list = np.abs(c_eig_list1-c_eig_list2)
        total_delta = np.sum(delta_c_eig_list)
        return bool(total_delta < 1e-8)
    ...
    def get_bond_list(self,contain_bond_order = True):
        ...
        return process.bo_matrix_to_bond_list(check_matrix,contain_bond_order)
    ...
    def get_formula(self,return_type = 'dict'):
        ...
        # When string formula is desired ...
        formula = ''
        # Sort by element ...
        element_sorted = dict(sorted(element_num.items(),key=lambda item: am.getZ(item[0]),reverse=True))
        for element_type in element_sorted:
            formula += element_type + str(element_sorted[element_type])
        return formula
    ...
    def get_minimal_data(self):
        ...
        data['mult'] = self.get_multiplicity()
        data['bo'] = self.get_bo_matrix()
        data['atom_chg'] = self.get_chg_list()
        data['coords'] = self.get_coordinate_list()
        return data        

    def get_rd_mol(self,method = 'SumofFragments',include_stereo = False):
        """
        ...
        chg_list = self.get_chg_list()
        bo_matrix = self.get_bo_matrix()
        if chg_list is None or bo_matrix is None:
            chg_list, bo_matrix = self.compute_chg_and_bo(method)
        if chg_list is None or bo_matrix is None:
            if chg_list is None: 
                print ('RDKit mol cannot be created because the formal charge is not well calculated ...')
                return None
            elif bo_matrix is None:
                print ('RDKit mol cannot be created because the bond order matrix is not well calculated ...')
                return None
        ...
        # Add bonds
        bond_list = process.bo_matrix_to_bond_list(bo_matrix)
        for bond in bond_list: 
    ...
    def print_coordinate_list(self,option='element'):
        ...
        for i in range(n):
            ...
            print_x = f"{coordinate[0]:.6f}"
            print_y = f"{coordinate[1]:.6f}"
            print_z = f"{coordinate[2]:.6f}"
    ...
    def get_valid_molecule(self,method='SumofFragments'):
        ...
        virtual_molecule = Molecule((new_z_list,bo_matrix,new_chg_list,1)) #non-radical species ...
        return virtual_molecule
    '''
    '''
    ...
    def make_3d_coordinates(self,num_conformer = 1,library = 'rdkit'):
        """
        Returns possible 3d molecular geometry using other libraries, mainly 'rdkit'

        :param library(str):
            Either 'rdkit' are possible. You can add your own found libraries for generating 3d structure 
    ...
    def sample_conformer(self):
        return self.sample_conformers(1)[0]

    def sample_conformers(self,n_conformer = 20,library = 'rdkit',rmsd_criteria = 1.0,check_geometry = True):
        conformer_list = []
        coordinates = self.make_3d_coordinates(n_conformer,library)
        #print (len(coordinates),'adfsdf')
        for coordinate_list in coordinates:
            ace_mol = Molecule((self.get_z_list(),self.get_adj_matrix(),self.get_chg(),self.get_multiplicity()))
...
class Intermediate(Molecule):
    ...
    """
    def __init__(self,data = None,atom_indices_for_each_molecule = None):
        molecule_list = None   
        self.number_list = []
        ...
        if type(data) is str:
            ...
            super().__init__(data)
            # Consider mol_chgs, mol_multiplicity
            save_molecule_list = True
            if 'mol_chg' in data and data['mol_chg'] is not None:
                mol_chg_list = data['mol_chg']
            else:
                save_molecule_list = False
            if 'mol_mult' in data and data['mol_mult'] is not None:
                mol_multiplicity_list = data['mol_mult']
            else:
                save_molecule_list = False
            if save_molecule_list:
                molecule_list = self.get_molecule_list()
                for i, molecule_list in enumerate(molecule_list):
                    molecule.chg = mol_chg_list[i]
                    molecule.multiplicity = mol_multiplicity_list[i]
                self.molecule_list = molecule_list    
        elif type(data) is list:
            if hasattr(data[0], '__len__'):
                super().__init__(data)
            else:  # Only molecule_list is formed when molecule_list is given as input ...
                super().__init__()
                molecule_list = data
        elif data is None:
            super().__init__(None)
        if molecule_list is not None: # Make every thing ... atom_list, adj, bo, chg
            ...
            atom_list = []
            # Also check keys in molecule_feature ...
            ...
            for molecule in molecule_list:
                ...
                if make:
                    atom_indices_for_each_molecule.append(list(range(cnt,cnt+n)))
                cnt += n
                for key in molecule.atom_feature:
                    if key not in present_keys:
                        present_keys[key] = [True]
                    else:
                        present_keys[key].append(True)
            atom_list = [None] * cnt
            atom_feature = dict()
            for key in present_keys:
                if all(present_keys[key]):
                    atom_feature[key] = np.zeros((cnt))
           
            mol_chg_list = []
            mol_multiplicity_list = []
            # Make adj, bo matrix ...
            ...
            for i,molecule in enumerate(molecule_list):
                ...
                # Generate atom list and atom feature
                for j in range(m):
                    atom_list[atom_indices[j]] = molecule_atom_list[j]
                    for key in atom_feature:
                        atom_feature[key][atom_indices[j]] = molecule_atom_feature[key][j]
                mol_chg = molecule.get_chg()
                if mol_chg is not None:
                    mol_chg_list.append(mol_chg)
                mol_multiplicity = molecule.get_multiplicity()
                if mol_multiplicity is not None:
                    mol_multiplicity_list.append(mol_multiplicity-1)
                # Update adj/bo matrix
                ...
                if bo_matrix is None:
                    ...
                    total_bo_matrix[index_function] = bo_matrix
                if adj_matrix is None:
                    update_adj_matrix = False
                else:
                    total_adj_matrix[index_function] = adj_matrix
            
            if len(mol_chg_list) == 0:
                self.chg = None
            else:
                self.chg = sum(mol_chg_list)
            if len(mol_multiplicity_list) == 0:
                self.multiplicity = None
            else:
                self.multiplicity = sum(mol_multiplicity_list) + 1 

            if update_bo_matrix:
                self.bo_matrix = total_bo_matrix
            elif update_adj_matrix:
    ...
    def set_molecule_list(self,molecule_list):
        pass
    ...
    def get_multiplicity(self):
        if self.multiplicity is not None:
            ...
            for molecule in molecule_list:
                mol_multiplicity = molecule.get_multiplicity()
                if mol_multiplicity is None:
                    return None
                else:
                    num_spin += mol_multiplicity - 1
            return (num_spin + 1)
    ...
    def get_adj_matrix(self):
        # If adj matrix exists, use that ... Other wise, get from atom indices ...         
        if self.adj_matrix is not None:
            return self.adj_matrix
        if self.bo_matrix is not None:
            return np.where(self.bo_matrix>0,1,0)
        # Otherwise, it must be recovered by atom indices ...
        n = len(self.atom_list)
        molecule_list = self.get_molecule_list()
        atom_indices_for_each_molecule = self.get_atom_indices_for_each_molecule()
        total_adj_matrix = np.zeros((n,n))
        for i, atom_indices in enumerate(atom_indices_for_each_molecule):
            molecule = molecule_list[i]
            adj_matrix = molecule.get_adj_matrix()
            if adj_matrix is None:
                return None
            reduce_function = np.ix_(atom_indices,atom_indices)
            total_adj_matrix[reduce_function] = adj_matrix
        return total_adj_matrix

    def get_bo_matrix(self):
        # If bo exist, use that one ...
        if self.bo_matrix is not None:
            return self.bo_matrix
        # Else, recover ...
        n = len(self.atom_list)
        ...
        for i, atom_indices in enumerate(atom_indices_for_each_molecule):
            ...
            if bo_matrix is None:
                return None
            reduce_function = np.ix_(atom_indices,atom_indices)
            total_bo_matrix[reduce_function] = bo_matrix
        return total_bo_matrix

    
    def compute_chg_and_bo(self,method='SumofFragments'):
        n = len(self.atom_list)
        total_chg_list = np.zeros((n))
        total_bo_matrix = np.zeros((n,n))
        molecule_list = self.get_molecule_list()
        #print ('indices',self.atom_indices_for_each_molecule)
        atom_indices_for_each_molecule = self.get_atom_indices_for_each_molecule()
        for i, molecule in enumerate(molecule_list):
            atom_indices = atom_indices_for_each_molecule[i]
            chg_list, bo_matrix = molecule.compute_chg_and_bo(method)
            if chg_list is None or bo_matrix is None:
                print ('Charge cannot be computed !!!')
                return None, None 
            reduce_function = np.ix_(atom_indices,atom_indices)
            total_chg_list[atom_indices] = chg_list
            total_bo_matrix[reduce_function] = bo_matrix
        return total_chg_list, total_bo_matrix
    ...
    def get_minimal_data(self):
        ...
        data['mult'] = self.get_multiplicity()
        molecule_list = self.get_molecule_list()
        data['mol_chg'] = np.array([molecule.get_chg() for molecule in molecule_list])
        data['mol_mult'] = np.array([molecule.get_multiplicity() for molecule in molecule_list])
        data['bo'] = self.get_bo_matrix()
        data['atom_chg'] = self.get_chg_list()
        data['coords'] = self.get_coordinate_list()
        return data        
    ...
    def get_id(self,method='ace'):
        ...
        if molecule_list is None:
            ...
            content = []
            for molecule in molecule_list:
                molecule_id = molecule.get_id()
                if molecule_id is None:
                    return None
                content.append(molecule_id)
            content.sort()
    ...
    def get_smiles(self,method = 'SumofFragments',find_stereocenter = 'N'):
        ...
        for molecule in molecule_list:
            ...
            if chg is None:
                print ('Charge of the molecule should be specified !!!')
                return None
            smiles = molecule.get_smiles(method,find_stereocenter)
            if smiles is None:
    ...
    def visualize(self,method='rdkit'):
        ...
        #print (self.get_coordinate_list())
        self.write_geometry('tmp.xyz')
        import os
        os.system('molden tmp.xyz')  
        os.system('rm tmp.xyz')
