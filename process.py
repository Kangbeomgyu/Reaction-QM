...
#from utils import compute_gurobi
...
def get_adj_matrix_from_distance(molecule,coeff = 1.30,grid = 0.05):
    """
    ...
    distance_matrix = spatial.distance_matrix(coordinate_list,coordinate_list)
    # Try while loop ...
    non_full_indices = list(range(n))
    start_coeff = 0.95
    end_coeff = coeff
    # Reset grid ...
    N = int((end_coeff - start_coeff)/grid) + 1 # Num scan ...
    grid = (end_coeff-start_coeff) / N 
    max_valency_list = molecule.get_max_valency_list()

    # Get maximum valency ...
    ratio_matrix = distance_matrix/radius_sum_matrix
    np.fill_diagonal(ratio_matrix,1000)

    for i in range(N+1):
        # Fill in smallest ratio for non_full_indices ...
        m = len(non_full_indices)
        current_coeff = start_coeff + i * grid
        reduce_function = np.ix_(non_full_indices, non_full_indices)
        index_function = {j:non_full_indices[j] for j in range(m)}
        reduced_ratio_matrix = ratio_matrix[reduce_function]
        new_bonds = np.where(reduced_ratio_matrix<current_coeff)
        #print (index_function)
        if len(new_bonds[0]) == 0:
            continue
        for j in range(len(new_bonds[0])):
            new_bonds[0][j] = index_function[new_bonds[0][j]]
        for j in range(len(new_bonds[1])):
            new_bonds[1][j] = index_function[new_bonds[1][j]]
        adj_matrix[new_bonds] = 1
        non_full_indices = np.where(np.sum(adj_matrix,axis=1)<max_valency_list)[0].tolist()
...
def get_ace_mol_from_minimal_data(minimal_data,object_type = 'molecule'): 
    ...
    chg_list = minimal_data['atom_chg']
    coordinate_list = minimal_data['coords']
    if object_type == 'molecule':
        molecule = chem.Molecule(minimal_data)
    else:
        molecule = chem.Intermediate(minimal_data)
    if chg is not None:
        molecule.chg = chg
...
def bo_matrix_to_bond_list(bo_matrix,contain_bond_order = True):
    total_bond_list = []
    if contain_bond_order:       
        bond_type = [1,2,3]
        check_matrix = bo_matrix
    else:
        bond_type = [1]
        check_matrix = np.where(bo_matrix>0,1,0) # Adj matrix
...
def molecule_to_intermediate(ace_mol):
    ...
    intermediate.chg = ace_mol.get_chg()
    intermediate.multiplicity = ace_mol.get_multiplicity()
    return intermediate
